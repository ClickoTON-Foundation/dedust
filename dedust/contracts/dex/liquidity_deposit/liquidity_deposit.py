...
class LiquidityDeposit:
    ...
    async def get_owner_address(provider: Provider) -> Address:
        stack = await provider.runGetMethod(address=self.address,
                                            method="get_owner_addr")
        return stack[0]["value"].read_msg_addr()
    
    async def get_pool_address(provider: Provider) -> Address:
        stack = await provider.runGetMethod(address=self.address,
                                            method="get_pool_addr")
        return stack[0]["value"].read_msg_addr()
    
    async def get_pool_params(provider: Provider) -> list:
        stack = await provider.runGetMethod(address=self.address,
    ...
    async def get_target_balances(provider: Provider) -> list:
        stack = await provider.runGetMethod(address=self.address,
                                            method="get_target_balances")
        return [stack[0]["value"], stack[1]["value"]]

    async def get_balances(provider: Provider) -> list:
        stack = await provider.runGetMethod(address=self.address,
                                            method="get_balances")
        return [stack[0]["value"], stack[1]["value"]]

    async def get_is_processing(provider: Provider) -> bool:
        stack = await provider.runGetMethod(address=self.address,
                                            method="is_processing")
        return bool(stack[0]["value"])
    
    async def get_minimal_lp_amount(provider: Provider) -> int:
        stack = await provider.runGetMethod(address=self.address,
